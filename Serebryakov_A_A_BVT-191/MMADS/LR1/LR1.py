...
Mvx = 7  # Масса входа (статическая), кг/с
...
R = 2.25 * 10**6  # Теплота парообразования вторичного пара, Дж/кг
...
def function(mvx, cvx, tpi):
    return (mvx * cvx) / (mvx + (Kt * F * (tpi - Tp) / (Ct * Tp - R)))
...
def plotC():
    ...
    plot.grid()
    plot.title("Концентрация выхода от концентрации входа")
    ...
    plot.plot(tArray, Cvix)
    plot.grid()
    plot.title("Концентрация выхода от массы входа")
    plot.xlabel('Масса')
    plot.ylabel('Концентрация выхода')
    plot.show()
    ...
    plot.plot(tArray, Cvix)
    plot.grid()
    plot.title("Концентрация выхода от температуры греющего пара")
